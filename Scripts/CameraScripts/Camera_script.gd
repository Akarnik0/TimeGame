...
func _unhandled_input(event: InputEvent):
	if event.is_action_pressed("show_map") && Is_on_character == true && Is_moving_camera == false:
		Move_camera_to_center(Get_level_center())
	elif event.is_action_pressed("show_map") && Is_on_character == false && Is_moving_camera == false:
